...
make_command_stream (int (*get_next_byte) (void *),
	...
	while((current_c = *(input_stream+input_iterator)) != '\0') {
		...
		switch(current_c) {
		     case '\n':
					//if(keyword_iterator != 0 && keyword_stream[keyword_iterator-1].type == WORD) {
						keyword_stream[keyword_iterator].word = NULL;
						keyword_stream[keyword_iterator].line = current_line;
						keyword_stream[keyword_iterator].type = NEWLINE;
						keyword_iterator++;
					//}
					/*current_line++;
					int counter = 1;
		...
		if(word_char(current_c)) {
			...
			if (strcmp(word_temp, "if") == 0){
					...
					free(keyword_stream[keyword_iterator].word);
					keyword_stream[keyword_iterator].word = NULL;
			}
