...
#define TASKBUFSIZ	65336	// Size of task_t::buf
...
int compute_digest(char *fn, char *digest)
{
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	if (t->total_written > 0) {
		...
		if (strlen(tracker_task->digest) > 0) {
			...
			if (compute_digest(t->disk_filename, check_digest) == 0) {
				message("* Error for '%s': digest could not be computed.\n", t->disk_filename);
